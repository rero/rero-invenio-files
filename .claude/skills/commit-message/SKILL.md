...
- Types: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`,
  `build`, `ci`, `revert`.
- Scope: the package under `rero_invenio_files/` that the change touches
  (`records`, `pdf`). Elsewhere, use the area, as the history already does —
  `files`, `deps`, `docs`, `dev`. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(files)!: ...`.
